...
class ShapeNetDataLoader:
    def __init__(self, cfg, ft=False, rot=False):
        self.dataset_taxonomy = None
        self.finetune = False
        if ft:
            self.finetune = True
            self.ft_rate = cfg.TRAIN.FINETUNE_RATE
            self.rendering_image_path_template = cfg.DATASETS.SHAPENET.TRAIN_RENDERING_PATH
            self.rendering_image_path_template_ft = cfg.DATASETS.SHAPENET.FINETUNE_RENDERING_PATH
        elif rot:
            self.rendering_image_path_template = cfg.DATASETS.SHAPENET.TEST_RENDERING_PATH
        else:
            self.rendering_image_path_template = cfg.DATASETS.SHAPENET.TRAIN_RENDERING_PATH
    ...
    def get_files_of_taxonomy(self, taxonomy_folder_name, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            if self.finetune:
                if sample_idx < int(len(samples) * self.ft_rate):
                    img_file_path = self.rendering_image_path_template_ft % (taxonomy_folder_name, sample_name, 0)
                    flag = True
                else:
            ...
            for image_idx in rendering_image_indexes:
                if self.finetune:
                    if flag:
                        img_file_path = self.rendering_image_path_template_ft % (
                        taxonomy_folder_name, sample_name, image_idx)
                    else:
