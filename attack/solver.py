  ...
  def ask(self):
    ...
    if self.average_baseline:
     ...
     # first population is every mu, then positive epsilon, then negative epsilon
        r = np.concatenate([np.zeros((self.num_k, self.num_params)), self.r_full])  #[num_k + 2 * batchsize, num_params]
        self.r_full = r
    ...
    if self.average_baseline:
     ...
     # first population is every mu, then positive epsilon, then negative epsilon
        ...
        F = np.concatenate([l, self.F_full])  #[num_k + 2 * batchsize, num_params]
        self.F_full = F
