...
def ours_wo_pool_search(args, model, dataloader, rot_data=None, mood='init', rotation_pool=None):
    device = torch.cuda.current_device()
    if rotation_pool is None:
        rotation_pool = dict()
    ...
    with torch.no_grad():
        ...
        if mood == 'warm_start':
            for temp in tqdm(range(1), total=1, desc='Searching for viewpoints for novel dataset'):
                ...
                for id in range(len(rot_data['taxonomy_names'])):
                    ...
                    sample_names.extend(rot_data['sample_names'][id])
                ...
                if taxonomy_names[batch_id] not in rotation_pool:
                    rotation_pool[taxonomy_names[batch_id]] = dict()
                    rotation_pool[taxonomy_names[batch_id]][sample_names[batch_id]] = [trans]
                else:
                    if sample_names[batch_id] not in rotation_pool[taxonomy_names[batch_id]]:
                        rotation_pool[taxonomy_names[batch_id]][sample_names[batch_id]] = [trans]
                    else:
                        rotation_pool[taxonomy_names[batch_id]][sample_names[batch_id]].append(trans)
        ...
        for batch_idx, (taxonomy_names, sample_names, data,
            ...
            if not os.path.exists(args.rotation_pool_path % (args.interval_degree, args.attack_model)):
                os.makedirs(args.rotation_pool_path % (args.interval_degree, args.attack_model))
            rotation_path = os.path.join(args.rotation_pool_path % (args.interval_degree, args.attack_model),
                                         'rotation_pool_%d.json' % dist.get_rank())
